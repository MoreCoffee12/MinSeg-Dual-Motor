...
if Online
    ...
    strPlotTitle ='ImpulseTest01_FS_100_TrueVert.mat';
    %strPlotTitle ='DataSet23_TuningComplete_GyroBW042_Q220_Q11_250_Q33_001_FS_020_Balancing.mat';
...
title({strPlotTitle 'Position Spectrum'},'Interpreter','None')
hold on
...
title('Average Pitch Angle Rate')
subplot(4,1,2)
dGyroUpper = iBuffData(6,:);
plot(dGyroUpper)
%ylim([-1 1])
title('Upper Pitch Angle Rate')
subplot(4,1,3)
dGyroLower = iBuffData(7,:);
plot(dGyroLower)
%ylim([-1 1])
...
title({strPlotTitle 'Upper Gyro Spectrum'},'Interpreter','None')
hold on
plot(ws,abs(Y),'k','LineWidth',1.0);
grid on
xlabel('Frequency, Hz')
